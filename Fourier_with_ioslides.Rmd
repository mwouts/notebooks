...
def plot_signal_and_fft(signal, name, N=20, asFigure=False):
    ...
    data = [go.Scatter(x=np.arange(-len(signal), len(signal)),
                       y=np.concatenate((signal,signal)), name='Discrete signal'),
            go.Scatter(x=np.arange(-len(signal), len(signal)),
                y=np.concatenate((signal_reconstructed,signal_reconstructed)), name='Signal reconstructed from truncated FFT'),
           go.Bar(x=x, y=y.real, 
